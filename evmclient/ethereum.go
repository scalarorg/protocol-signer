...
func (c *EthereumClient) LatestFinalizedBlockNumber(ctx context.Context, confirmations uint64) (*big.Int, error) {
	...
	var finalizedBlockNumber = new(big.Int)
	return finalizedBlockNumber.SetUint64(blockNumber).Sub(finalizedBlockNumber, big.NewInt(int64(confirmations))).Add(finalizedBlockNumber, big.NewInt(1)), nil
}
